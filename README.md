...
这里原本是「数字成绩条」（场次 / 时长 / 粉丝 / 商家），因为**不放未经核实的数据**，改成了定位标签：
...
改文字就在 `.stats` 区域里改 `<span class="stat-tag">` 与 `<span class="stat-label">`。

#### 以后有真实数据，想换回数字版

数字滚动的**脚本和样式都还留着**，把卡片内容换成下面这样就会自动生效：

```html
<div class="stat">
  <span class="stat-num" data-count="300">0</span><span class="stat-suffix">+</span>
  <span class="stat-label">直播场次</span>
</div>
```

- `data-count` = 滚动目标值
- `stat-suffix` = 后缀（`+` / `万` / `%`…）
- 进入视口时从 0 滚上去，若 3.5 秒未触发会兜底直接显示目标值
